(function (PLUGIN_ID) {
  ...
  function getViewFilter(viewId, callback) {
    kintone.api(
      ...
      console.log('[MobileSearch] View Filter:', filter);
      callback(filter);
    }).catch(function (err) {
      console.error('[MobileSearch] View Filter Error:', err);
      callback('');
    });
  }
  ...
  function collectTextsFromRecord(record, fieldCodes) {
    ...
    fieldCodes.forEach(function (code) {
      ...
      // If a plain SUBTABLE code is passed, search all its columns
  ...
  function fetchAllRecords(offset, collected) {
    ...
    return kintone.api(
      ...
      recordsLoaded = true;
      console.log(
        '[MobileSearch] Records loaded:', allRecords.length,
        '| Search fields:', searchFields
      );
      return collected;
    }).catch(function (err) {
      console.error('[MobileSearch] fetchAllRecords Error:', err);
      recordsLoaded = true;
    });
  }
  ...
  function filterRecords(searchText) {
    ...
    console.log('[MobileSearch] Filter terms:', terms, '| Total records:', allRecords.length);
    ...
    console.log('[MobileSearch] Matched:', matches.length, 'for "' + searchText + '"');
    return matches;
  }
  ...
  function reload(query, text) {
    ...
    console.log('[MobileSearch] Reload URL:', url);
  ...
  kintone.events.on('mobile.app.record.index.show', function (event) {
    ...
    if (!searchFields.length) {
      console.error('[MobileSearch] No search fields configured!');
      return event;
    }
    ...
    if (document.getElementById('mobile-search-wrapper')) {
      recordsLoaded = false;
      allRecords    = [];
    ...
    wrapper.style.cssText  =
      'padding:10px;background:#f5f5f5;border-bottom:1px solid #d1d1d1;';
    ...
    function performSearch() {
      ...
      if (ids.length > MAX_QUERY_IDS) {
        alert(
          ...
          'Please refine. Use spaces for AND search.'
        );
        return;
      }
      ...
      console.log('[MobileSearch] Final Query:', finalQuery);
    ...
    // Restore search text from sessionStorage or URL
